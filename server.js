...
// Sessions actives en mémoire : userId -> sessionId
...
app.get('/setup-admin', async (req, res) => {
  try {
    ...
    res.send(`<div style="background:#020510;color:#00f5ff;font-family:monospace;padding:40px;min-height:100vh;">
      <h2>✅ Admins créés !</h2>
...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    // Connexion sur un seul appareil (sauf admin)
    if (user.role !== 'admin') {
...
app.get('/me', checkAuth, async (req, res) => {
  ...
  // Vérifier session unique (sauf admin)
  if (user.role !== 'admin') {
    if (activeSessions[user._id] && activeSessions[user._id] !== req.session.sessionId) {
      req.session.destroy();
      return res.status(401).json({ error: 'session_conflict', message: 'Votre compte est connecté sur un autre appareil.' });
    }
    if (user.banned) {
      req.session.destroy();
...
// Restreindre (bloquer les analyses)
